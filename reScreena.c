...
// j'ai fais quasi rien dessus, c'est les personnes au dessus qu'il faut remercier
...
int hook_func1(int r1, ImgParam2 *param2, ImgParam3 *param3) {
	...
	if(param2->type == 2) {
		...
		param2->path_len = sce_paf_snprintf(param2->outpath, PATH_MAX - sizeof(fn) - param2->img_ext_len, "photo0:/ALL/%s", titlename);
...
void _start() __attribute__ ((weak, alias ("modulea_start")));

int modulea_start(SceSize argc, const void *args) {
...
int modulea_stop(SceSize argc, const void *args) {
